(function () {
    'use strict';
    angular.module('app').controller('QualificacaoController', ['Qualificacao', '$state', 'Workspace', '$q', QualificacaoController]);

    var state = "qualificacao"

    function QualificacaoController(Qualificacao, $state, Workspace, $q) {
        var vm = this;
        ...
        function load(query) {
            vm.promise = Qualificacao.query(query, success).$promise;
            loadCount()
        ...
        function loadCount() {
            Qualificacao.count(vm.query).$promise.then(function (e) {
                vm.count = e.count
            });
