(function () {
    'use strict';
    angular.module('app').controller('FuncionarioAcidenteController', ['FuncionarioAcidente', '$state', 'Workspace', '$q', FuncionarioAcidenteController]);

    var state = "funcionario-acidente"

    function FuncionarioAcidenteController(FuncionarioAcidente, $state, Workspace, $q) {
    var vm = this;
    ...
    vm.list = []
    vm.selectedItems = []
    Workspace.title = "Acidentes dos Colaboradores"
    ...
    function load(query) {
        vm.promise = FuncionarioAcidente.query(query, success).$promise;
        loadCount()
    ...
    function loadCount() {
        FuncionarioAcidente.count(vm.query).$promise.then(function (e) {
            vm.count = e.count
        });
