(function () {
    'use strict';
    angular.module('app').controller('SindicatoController', ['Sindicato', '$state', 'Workspace', '$q', SindicatoController]);

    var state = "sindicato"

    function SindicatoController(Sindicato, $state, Workspace, $q) {
        var vm = this;
        ...
        function load(query) {
            vm.promise = Sindicato.query(query, success).$promise;
            loadCount()
        ...
        function loadCount() {
            Sindicato.count(vm.query).$promise.then(function (e) {
                vm.count = e.count
            });
