(function () {
    'use strict';
    angular.module('app').controller('TabelaController', ['Tabela', '$state', 'Workspace', '$q', TabelaController]);

	var state = "tabela"
	
    function TabelaController(Tabela, $state, Workspace, $q) {
        var vm = this;
        ...
        function load(query) {
            vm.promise = Tabela.query(query, success).$promise;
            loadCount()
        ...
        function loadCount() {
            Tabela.count(vm.query).$promise.then(function (e) {
                vm.count = e.count
            });
