(function () {
    'use strict';
    angular.module('app').controller('QuestaoController', ['Questao', '$state', 'Workspace', '$q', QuestaoController]);

    var state = "questao"

    function QuestaoController(Questao, $state, Workspace, $q) {
        var vm = this;
        ...
        function load(query) {
            vm.promise = Questao.query(query, success).$promise;
            loadCount()
        ...
        function loadCount() {
            Questao.count(vm.query).$promise.then(function (e) {
                vm.count = e.count
            });
