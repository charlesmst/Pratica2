(function () {
    'use strict';
    angular.module('app').controller('CandidatoController', ['Candidato', '$state', 'Workspace', '$q', CandidatoController]);

    var state = "candidato"

    function CandidatoController(Candidato, $state, Workspace, $q) {
        var vm = this;
        ...
        function load(query) {
            vm.promise = Candidato.query(query, success).$promise;
            loadCount()
        ...
        function loadCount() {
            Candidato.count(vm.query).$promise.then(function (e) {
                vm.count = e.count
            });
