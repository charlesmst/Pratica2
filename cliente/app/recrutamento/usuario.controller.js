(function () {
    'use strict';
    angular.module('app').controller('UsuarioController', ['Usuario', '$state', 'Workspace', '$q', UsuarioController]);

    var state = "usuario"

    function UsuarioController(Usuario, $state, Workspace, $q) {
        var vm = this;
        ...
        function load(query) {
            vm.promise = Usuario.query(query, success).$promise;
            loadCount()
        ...
        function loadCount() {
            Usuario.count(vm.query).$promise.then(function (e) {
                vm.count = e.count
            });
