(function () {
    'use strict';
    angular.module('app').controller('CboController', ['Cbo', '$state', 'Workspace', '$q', CboController]);

    var state = "cbo"

    function CboController(Cbo, $state, Workspace, $q) {
        var vm = this;
        ...
        function load(query) {
            vm.promise = Cbo.query(query, success).$promise;
            loadCount()
        ...
        function loadCount() {
            Cbo.count(vm.query).$promise.then(function (e) {
                vm.count = e.count
            });
